...
- Particle shaders now receive shadows- `CameraData.volumeLayerMask` and `CameraData.volumeTrigger` have been deprecated.
...
- Fixed camera inverse view-projection matrix in XR mode, depth-copy and color-copy passes.- Fixed an issue where a depth texture was always created when post-processing was enabled, even if no effects made use of it.
