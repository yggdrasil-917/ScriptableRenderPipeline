...
{
    // TODO - Make inheritence work correctly

    using CoreEditorUtils = UnityEditor.Rendering.CoreEditorUtils;
    
    [CustomEditor(typeof(ToonPipelineAsset))]
    public class ToonPipelineAssetEditor : Editor
    {
        ...
        void DrawStylizationSettings()
        {
            EditorGUILayout.LabelField("Stylization", EditorStyles.boldLabel);
            EditorGUI.indentLevel++;
            ...
            EditorGUI.PrefixLabel(labelRect, EditorGUIUtility.TrTextContent("Profile", "A reference to a profile asset."));
            ...
            using (new EditorGUI.DisabledScope(multiEdit))
            {
                if (GUI.Button(buttonNewRect, EditorGUIUtility.TrTextContent("New", "Create a new profile."), showCopy ? EditorStyles.miniButtonLeft : EditorStyles.miniButton))
                {
                    ...
                    var asset = VolumeUtils.CreateVolumeProfile("Assets", "Pipeline Global Profile");
                    m_Profile.objectReferenceValue = asset;
                    assetHasChanged = true;
                }

                if (showCopy && GUI.Button(buttonCopyRect, EditorGUIUtility.TrTextContent("Clone", "Create a new profile and copy the content of the currently assigned profile."), EditorStyles.miniButtonRight))
                {
