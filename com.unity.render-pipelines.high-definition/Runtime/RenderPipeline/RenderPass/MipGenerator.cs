    ...
    public class MipGenerator
    {
        ...
        public int RenderColorGaussianPyramid(CommandBuffer cmd, Vector2Int size, Texture source, RenderTexture destination)
        {
            ...
            if (preferFragment)
            {
                ...
                m_PropertyBlock.SetVector(HDShaderIDs._SrcUvLimits, Vector4.zero);
                ...
                while (srcMipWidth >= 8 || srcMipHeight >= 8)
                {
                    ...
                    using (new ProfilingSample(cmd, "Blur horizontal", CustomSamplerId.ColorPyramid.GetSampler()))
                    {
                        ...
                        m_PropertyBlock.SetVector(HDShaderIDs._SrcUvLimits, new Vector4(
                            sourceScaleX * (dstMipWidth - 0.5f) / m_TempDownsamplePyramid[kernelIndex].rt.width,
                            sourceScaleY * (dstMipHeight - 0.5f) / m_TempDownsamplePyramid[kernelIndex].rt.height,
                            sourceScaleX / m_TempDownsamplePyramid[kernelIndex].rt.width,
                            0f));
                    ...
                    using (new ProfilingSample(cmd, "Blur vertical", CustomSamplerId.ColorPyramid.GetSampler()))
                    {
                        ...
                        m_PropertyBlock.SetVector(HDShaderIDs._SrcScaleBias, new Vector4(scaleX, scaleY, 0f, 0f));
                        m_PropertyBlock.SetVector(HDShaderIDs._SrcUvLimits, new Vector4(
                            sourceScaleX * (dstMipWidth - 0.5f) / m_TempColorTargets[kernelIndex].rt.width,
                            sourceScaleY * (dstMipHeight - 0.5f) / m_TempColorTargets[kernelIndex].rt.height,
                            0f,
                            sourceScaleY / m_TempColorTargets[kernelIndex].rt.height
                            ));
                        m_PropertyBlock.SetFloat(HDShaderIDs._SourceMip, 0);
